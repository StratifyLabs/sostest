...
bool MqTest::execute_class_api_case(){
	...
	mq_oflag = Mq::flag_create;
	attr_flag = MqAttributes::open_read_write;
	attr_mq_maxmsg = 10;
	attr_msgsize = 12;
	attr_curmsgs = 0;
...
bool MqTest::execute_api_mq_attr_case(MqAttributes * mq_attr){
	...
	long attr_flag,attr_mq_maxmsg,attr_msgsize,attr_curmsgs;
	MqAttributes mq_uno;
	attr_flag = MqAttributes::open_nonblock;
	attr_mq_maxmsg = 5;
	attr_msgsize = 5;
	...
	attr_flag = MqAttributes::open_read_only;
	mq_uno.set_flags(attr_flag);
...
static void * thread_1(void * args){
	...
	(void) args;
	if (mq_answer.create(
				mq_name_thread_answer,
				mq_oflag,
				fs::Permissions(mode),
				MqAttributes::open_read_write
				,20,16)>=0){
...
Mq::flags get_o_flags(int i){
	switch (i%3){
		case 0:
			return Mq::flag_create;
		case 1:
			return Mq::flag_exclusive;
		case 2:
			return Mq::flag_exclusive;
		default:
			return Mq::flag_create;
