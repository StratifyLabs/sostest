...
bool SocketTest::execute_class_api_case(){
	...
	close_case();
	open_case("Socket Option case");
	family = SocketAddressInfo::FAMILY_INET;
	if(!execute_socket_option_case()){
		print_case_failed("raw socket case failed");
		print_case_failed("Failed %s:%d", __PRETTY_FUNCTION__, __LINE__);
	}
	close_case();
...
*/
			...
			print_case_message("close udp server %d", __LINE__);
...
void * SocketTest::listen_on_localhost(){
	...
	local_host_listen_socket << SocketOption().socket_reuse_address() << SocketOption().socket_reuse_port();
...
bool SocketTest::execute_socket_address_info_case(){
	...
	print_case_message("free");
	list.free();
	print_case_message("clear");
	list.clear();
	print_case_message("fetch");
...
bool SocketTest::execute_socket_address_case(){
	...
	ipv4_in.sin_family = SocketAddressInfo::FAMILY_INET;

	for(u8 i = 0;i<16;i++){
		ipv6_in.sin6_addr.s6_addr[i] = IP_ADDRESS;
	}
	ipv6_in.sin6_port = htons(port);
	ipv6_in.sin6_family = SocketAddressInfo::FAMILY_INET6;
	...
	if( socket_address_info_ipv6.port() != port ){
		print_case_failed("Failed %s:%d (%d != %d)",
								__PRETTY_FUNCTION__,
								__LINE__,
								socket_address_info_ipv6.port(),
								port);
	}
	...
	if(socket_address_ipv6.port()!=port){
		print_case_failed("Failed %s:%d (%d != %d)", __PRETTY_FUNCTION__, __LINE__, socket_address_ipv6.port(), port);
	}

	if( socket_address_zero.is_valid() ){
		print_case_failed("Failed %s:%d (%d != %d)", __PRETTY_FUNCTION__, __LINE__, socket_address_zero.port(), port);
	}
...
bool SocketTest::execute_socket_option_case(){
	print_case_message("option test");
	return case_result();
	SocketAddressInfo address_info(family,SocketAddressInfo::TYPE_DGRAM,\
											 SocketAddressInfo::PROTOCOL_UDP,AI_PASSIVE);
	...
	if(list.size()<=0){
		...
		if( local_host_socket_udp_client.bind_and_listen(localhost_udp_address_client) < 0 ){
			print_case_failed("Failed %s:%d", __PRETTY_FUNCTION__, __LINE__);
		}
		...
		socket_option.socket_receive_timeout(clock_time);
		local_host_socket_udp_client << socket_option;
		Data read_data(256);
		/*read from address, we know answer will't be */
		SocketAddress socket_address_two(address_info);
		socket_address_two.set_protocol(SocketAddressInfo::PROTOCOL_UDP);

		if(local_host_socket_udp_client.read(read_data,socket_address_two)<0){
			print_case_failed("Failed %s:%d", __PRETTY_FUNCTION__, __LINE__);
		}
		clock_time = Clock::get_time();
		clock_time_add.set(0,50*1000*1000);
		...
		socket_option.socket_send_timeout(clock_time);
		local_host_socket_udp_client << socket_option;
		String send("option");
		if(local_host_socket_udp_client.write(send,socket_address_two)<0){
			print_case_failed("Failed %s:%d", __PRETTY_FUNCTION__, __LINE__);
		}

		Socket local_host_socket_udp_server;
		if(local_host_socket_udp_server.create(localhost_udp_address_client)!=0){
			print_case_failed("Failed %s:%d", __PRETTY_FUNCTION__, __LINE__);
		}
		socket_option.socket_reuse_address();
		socket_option.socket_reuse_port();
		local_host_socket_udp_server << socket_option;
		//reuse port from "local_host_socket_udp_client" to new user - "local_host_socket_udp_server"
		if( local_host_socket_udp_server.bind_and_listen(localhost_udp_address_client) < 0 ){
			print_case_failed("Failed %s:%d", __PRETTY_FUNCTION__, __LINE__);
		}
		local_host_socket_udp_client.close();
		local_host_socket_udp_server.close();
...
bool SocketTest::execute_socket_case_udp(){
	...
	len = local_host_socket_udp_server.read(reply.to_void(), reply.size(), &ai_addr, &ai_addrlen);
	if( len < 0 ){
		print_case_failed("Failed %s:%d", __PRETTY_FUNCTION__, __LINE__);
		return false;
	}
	...
	print_case_message("read '%s' from udp socket %d", reply.to_char(), __LINE__);
	...
	print_case_message("next socket address");

	//read two    int read(var::Data & data, SocketAddress & address);
	SocketAddress socket_address_two(address_info);
	print_case_message("set protocol udp");
	socket_address_two.set_protocol(SocketAddressInfo::PROTOCOL_UDP);
	reply.fill(0);
	print_case_message("about to read %p (%d)", reply.to_void(), reply.size());
	fflush(stdout);
	...
	if( test != reply.to_char() ){
		print_case_failed("Failed %s:%d ('%s' != '%s')", __PRETTY_FUNCTION__, __LINE__, test.cstring(), reply.to_char());
	}
	...
	String reply_string(reply.to_char(), len);
	...
	print_case_message("read '%s' from udp socket", reply_string.cstring());
	test.to_lower();//value must be changed
	if( test != reply_string ){
		print_case_failed("Failed %s:%d ('%s' != '%s')", __PRETTY_FUNCTION__, __LINE__, test.cstring(), reply_string.cstring());
	}
	print_case_message("wait 100ms");
	Timer::wait_milliseconds(100);
	print_case_message("close udp client %d", __LINE__);
	local_host_socket_udp_client.close();
	print_case_message("close udp server %d", __LINE__);
	local_host_socket_udp_server.close();
...
bool SocketTest::execute_socket_case_raw(){
	...
	socket_option = socket_option.socket_send_size(64);
	ClockTime clock_time,clock_time_add;
	clock_time = Clock::get_time();
	clock_time_add.set(0,50*1000*1000);
	clock_time +=clock_time_add;
	socket_option = socket_option.socket_receive_timeout(clock_time);
	Vector<SocketAddressInfo> list = address_info.fetch_node("stratifylabs.co");
	if(list.size()<=0){
		print_case_failed("Failed %s:%d", __PRETTY_FUNCTION__, __LINE__);
		return false;
...
void * SocketTest::listen_on_localhost_udp(){
	...
	if( test ){
		...
		//send two
		String send_data(reply.to_char(), len_hand);
		send_data.to_upper();//change sending data
		Timer::wait_milliseconds(100);
		print_case_message("%d->write udp client", __LINE__);
		len_hand = local_host_socket_udp_client.write(send_data.to_void(),send_data.length(),localhost_udp_address_client);
		if(len_hand  < 0 ){
			print_case_failed("Failed to write client socket ");
			return 0;
		}
		...
		len_hand = local_host_socket_udp_client.write(send_data.to_char(),send_data.length(),localhost_udp_address_client);
		if(len_hand  < 0 ){
			print_case_failed("Failed to write client socket ");
			return 0;
		}
	...
	print_case_message("return from thread");
	return 0;
}
...
*/
	...
	while(thread_running){
		...
		if( len_hand < 0 ){
			...
			print_case_message("--len ping udp %d", len_hand);
		}
	}
	print_case_message("--close udp client thread");
	local_host_socket_udp_client.close();
	print_case_message("--close udp client server");
	local_host_socket_udp_server.close();
	return 0;
}
